...
우선 2가지의 경우의 수를 생각했다. 45를 뺀 `minute` 의 값이 양수일때와 음수일때이다.
<br>양수일때는 입력 받은 `hour`과`minute`의 값이 그대로 출력되면 문제없다. 반대로 음수일때 다시 2가지 경우의 수가 생긴다. `hour`의 값이 0이라면 23을 출력하면 되고, 그 외의 값에는 입력 받은 값에서 1을 빼면 된다. 그리고 `minute`은 나온 음수의 값을 60에서 빼주면 출력값이 나온다.
